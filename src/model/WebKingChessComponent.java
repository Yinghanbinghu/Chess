 ...
 * 这个类表示国际象棋里面的王
 */
public class WebKingChessComponent extends WebChessComponent {
    ...
    public WebKingChessComponent(ChessboardPoint chessboardPoint, Point location, ChessColor color, WebClickController listener, int size) {
...
//        g.drawImage(rookImage, 0, 0, getWidth() - 13, getHeight() - 20, this);
    ...
    private boolean KingMeet(WebChessComponent[][] chessComponents, int row, int col){
        int i,j;
        i=row-1;j=col;
        if(i<8&&i>=0&&j<8&&j>=0&&chessComponents[i][j] instanceof WebKingChessComponent &&chessComponents[i][j].getChessColor()!=this.getChessColor()) return false;
        i=row-1;j=col+1;
        if(i<8&&i>=0&&j<8&&j>=0&&chessComponents[i][j] instanceof WebKingChessComponent &&chessComponents[i][j].getChessColor()!=this.getChessColor()) return false;
        i=row-1;j=col-1;
        if(i<8&&i>=0&&j<8&&j>=0&&chessComponents[i][j] instanceof WebKingChessComponent &&chessComponents[i][j].getChessColor()!=this.getChessColor()) return false;
