 ...
 * 这个类表示国际象棋里面的卒
 */
public class WebPawnChessComponent extends WebChessComponent {
    /**
    ...
    public WebPawnChessComponent(ChessboardPoint chessboardPoint, Point location, ChessColor color, WebClickController listener, int size) {
    ...
    @Override
    public boolean canMoveTo(WebChessComponent[][] chessComponents, ChessboardPoint destination, int step) {
...
//        g.drawImage(rookImage, 0, 0, getWidth() - 13, getHeight() - 20, this);
    ...
    @Override
    public ArrayList<ChessboardPoint> ChessCanMove(WebChessComponent[][] chessComponents, int step) {
    ...
    public ArrayList<ChessboardPoint> EatPassPawn(WebChessComponent[][] chessComponents, int step) {
        ArrayList<ChessboardPoint> EatPassPawn = new ArrayList<>();
        int row = this.getChessboardPoint().getX(), col = this.getChessboardPoint().getY();
        if (col + 1 <=7 && chessComponents[row][col + 1] instanceof WebPawnChessComponent && chessComponents[row][col + 1].getChessColor() != this.getChessColor() && chessComponents[row][col + 1].getTwoStep() + 1 == step) {
        ...
        if (col - 1 >= 0 && chessComponents[row][col - 1] instanceof WebPawnChessComponent && chessComponents[row][col - 1].getChessColor() != this.getChessColor() && chessComponents[row][col - 1].getTwoStep() + 1 == step) {
            if (this.getChessColor() == ChessColor.BLACK) EatPassPawn.add(new ChessboardPoint(row + 1, col - 1));
            else if (this.getChessColor() == ChessColor.WHITE) EatPassPawn.add(new ChessboardPoint(row - 1, col - 1));
        }
        return EatPassPawn;
